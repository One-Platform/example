    ...
    <div class="row">
        ...
        <div class="col-md-9" role="main">
            ...
            <div class="bs-docs-section">
                <h1 id="introduction">1. 缓存组件简介</h1>
                <p>本缓存组件是一个轻量级的缓存框架，吞吐量大，性能高，接口简单易用。当前的版本只支持本地内存缓存，在后续的版本中会陆续支持磁盘缓存，分布式
                    缓存，远程缓存等。</p>
            </div>

            <div class="bs-docs-section">
                <h1 id="environment">2. 环境搭建</h1>
                <p>对于非maven项目，需要添加相应的jar包：one-cache-0.0.1-beta.jar;</p>
                <p>对于maven项目，需要在pom.xml文件中添加如下依赖：</p>
                <pre><code>&lt;dependency&gt;
            ...
            <div class="bs-docs-section">
            <h1 id="quickstart">3. 使用说明</h1>
            <h2 id="buildcache">3.1 构架Cache</h2>
            <p>缓存会自填充，即：如果当前缓存中有指定的key对应的value，且该value有效，那么直接返回value；如果没有指定的key，那么调用给定的查询方法去获取value，返回value并填充缓存。</p>
            <p>提供了两种构建缓存的方式：</p>
            <ol>
                <li>使用CacheLoader，针对整个cache定义一个原子操作，根据给定的key获取value。</li>
                <li>
                    <p>使用Callable，此方式比较灵活，允许在get的时候指定如何获取value。 </p>
<pre><code>/**
    ...
    */
    ...
    return &quot;your query&quot;;
            ...
            <h2 id="cacheparameters">3.2 缓存参数介绍</h2>
            <ol>
                <li>可以选择性的调用CacheBuilderSelector的某些方法，为自己的缓存设置一些基本参数。</li>
                <li>
                    <p>缓存暂不支持选择清除策略，默认为LRU（最近最少使用）。 </p>
<pre><code>/**
    ...
    */
    ...
    CacheLoader&lt;? super K1, V1&gt; loader);
            ...
            <h2 id="callinterfaces">3.3 缓存的接口调用</h2>
            <p>缓存构建完成后，只需调用Cache和LoadingCache接口提供的方法，就可以进行相应的缓存操作。</p>
            <ol>
                <li>
                    <p>Cache接口提供了缓存基本的增删改查操作，在调用时再指定原子操作，是Callable方式的。	</p>
<pre><code>package com.sinosoft.one.cache;
    ...
    */
    void cleanUp();
                ...
                <li>
                    <p>LoadingCache接口扩展了Cache接口，在构建缓存时需要提供默认的原子操作，是CacheLoader方式的。</p>
<pre><code>package com.sinosoft.one.cache;
