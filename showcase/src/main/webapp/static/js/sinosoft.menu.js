			...
			function xmlLoopItem(menuItem){
				...
				menuItem.children().each(function(){
					if($(this).get(0).tagName == "group"){
						...
						if($(this).attr("icon") != undefined){
							$("a", $itemCode).append("<span class='bg_img' style=background-image:url('"+$(this).attr("icon")+"')></span>");
						} else {
							$("a", $itemCode).append("<span class='bg_img'></span>");	
						}
						if($(this).attr("callback")){
				...
				if( menuItem.MenuItems != undefined) {
					...
					for(var i = 0; i < menuItem.MenuItems.length; i++) {
						...
						if(menuItem.MenuItems[i].icon != undefined) {
							$("a", $itemCode).append("<span class='bg_img' style=background-image:url('"+menuItem.MenuItems[i].icon+"')></span>");
						} else {
							$("a", $itemCode).append("<span class='bg_img'></span>");	
						};						
			...
			function htmlLoopItem(itemHtml){
				...
				$(itemHtml).children().each(function(){			
					if($(this).get(0).tagName == "LI") {
						...
						if($(this).children("img").length > 0) {
							$("a", $itemCode).append("<span class='bg_img' style=background-image:url('"+$("img", this).attr("src")+"')></span>");
						} else {
							$("a", $itemCode).append("<span class='bg_img'></span>");
						}
						if($(this).attr("callback")){
