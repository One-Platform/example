...
Spring.buildDocumentationWidget = function (documentationEl, project) {
    new Spring.DocumentationWidgetView({el: documentationEl, model: project, template: $("#project-documentation-widget-template").text()}).render();
}
...
Spring.Release.prototype = {statusIconClass: function () {
    if (this.preRelease) {
...
Spring.DocumentationWidgetView = Backbone.View.extend({initialize: function () {
...
Spring.SnippetView = Backbone.View.extend({initialize: function () {
    ...
    this.combinedTemplate = _.template("<div class=\"highlight\"><pre><code>" +
        downloadTemplate.find("code:first").html() + "{@ if (repository) { @}" +
        repositoryTemplate.find("code:first").html() + "{@ } @}" + "</code></pre></div>");
    _.bindAll(this, "render");
}, render: function () {
...
Spring.QuickStartSelectorView = Backbone.View.extend({events: {"change .selector": "renderActiveWidget",
    //增加class为js-item的事件
    "click .js-item": "changeDownloadSource"}, initialize: function () {
    this.template = _.template(this.options.template);
    ...
    this.activeWidget = new Spring.SnippetView({el: this.snippetWidgetEl, model: this.model.releases[this.$('.selector :selected').val()], snippetType: this.$('.js-active').data('snippet-type')});
