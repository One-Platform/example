			...
			function createTrShadow(e) {
				...
				if($(".rowEditorDiv", $gView).length > 0) {
					...
					if(thisIndex < allTrNum - 2 || thisIndex == 1 || thisIndex == 2) {
						...
						$thisEditor.animate({ top: top - 2 + $gView.scrollTop()}, 200 );
					}
					...
					$(this).find("td").each(function(){
						...
						if($(this).find("input[type='checkbox']").length > 0) {
							...
							if(flag) {
								$innerDiv.append($(this).text()).width(tdWidth);
								$thisEditor.append($innerDiv);
								flag = false;
							} else {
								...
								if(defaults.colums[colVal].type == "date") {
									...
									$innerDiv.append($(this).html()).width(tdWidth).css("margin-left","1px").css("margin-top", (thisHeight - tem[0])/2 + 1);
									$thisEditor.append($innerDiv);
								} else {
									$innerDivInput = $("<input type='text' />").val($(this).text()).width(tdWidth - 6);
									$innerDiv.append($innerDivInput).width(tdWidth).css("margin-left","1px").css("margin-top", (thisHeight - tem[0])/2 + 1);
					...
					success: function(msg){
						if(msg == "success") {
							$.ajax({
								...
								dataType: "json",
								type: "POST",
								data:"pageNo=" + pB-1 + "&rowNum=" + defaults.rowNum,
								error: function (XMLHttpRequest, errorThrown) {
									alert("数据加载出错！" + errorThrown);
								},
									...
									if(!defaults.callback.afterDelData()){
										...
										if($(".grid_view").find("tr").length > 0) {
											$.ajax({
												...
												url: defaults.url,
												dataType: "json",
												data:"pB=" + pB + "&rowNum=" + defaults.rowNum,
											...
											$.ajax({
												...
												url: defaults.url,
												dataType: "json",
												data:"pB=" + pB + 2 + "&rowNum=" + defaults.rowNum,
												success: function(loadData) {
													var on = "unclick";
													if(pB != 1) {
														pB = pB - 1;
